...
public class VideoGenerator1 {
   ...
   private static  IRational FRAME_RATE=IRational.make(1,1);
    ...
    public static void main(String[] args) {
        ...
        for (int index = 0; index <= listOfFiles.length-1; index++) {
            BufferedImage screen = getImage(index);
            BufferedImage bgrScreen = convertToType(screen, BufferedImage.TYPE_3BYTE_BGR);
         writer.encodeVideo(0, bgrScreen, 3000*index, TimeUnit.MILLISECONDS);
         //if(index==listOfFiles.length-1) writer.encodeVideo(0, bgrScreen, 3000*(index+1), TimeUnit.MILLISECONDS);
        }
       writer.flush();
